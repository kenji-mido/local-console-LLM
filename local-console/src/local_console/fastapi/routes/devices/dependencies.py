...
def devices_controller(
    config: InjectGlobalConfig, device_service: InjectDeviceServices
) -> DevicesController:
    return DevicesController(config, device_service)


InjectDeviceController = Annotated[DevicesController, Depends(devices_controller)]
