...
from mocked_device.mock_v2.filters.rpc import RPCFilterV2
...
class RebootListener(TopicListener):
    def __init__(self, device: MockDevice):
        self._device = device
        if isinstance(device, MockDeviceV1):
            self.filter = RPCFilterV1()
        if isinstance(device, MockDeviceV2):
            self.filter = RPCFilterV2()

    def handle(self, message: TargetedMqttMessage) -> None:
        command = self.filter.filter(message)
        if command and command.method == "Reboot":
            logger.debug("Rebooting mocked device")
            self._device.reboot(command)
